...
//			slide_obj.imgthumb = jQuery('img', slide).src;
		...
		jQuery('#carousel-ck-slides').val(JSON.stringify(slides_list).replace(/"/g, '|qq|'));
	}
	...
	jQuery('#carousel-ck-params').val(JSON.stringify(params_obj).replace(/"/g, '|qq|'));
	jQuery('#carouselck-edit').submit();
}
...
function load_carouselck_demo_data() {
	if ( validate_data() === false ) {
		return;
	}
	...
	jQuery('#carousel-ck-slides').val(jQuery('#demo-slides').val());
	jQuery('#carousel-ck-params').val(jQuery('#demo-params').val());
	jQuery('#carouselck-edit').submit();
}
